...
const renderSelectedPopularProducts = async selectedCategoryId => {
  ...
  initProducts(document.querySelector('.products__list'), selectedProductId => {
    const selectedProduct = popularProducts.find(product => product.id === Number(selectedProductId));
...
const addSliders = () => {
  ...
  $(document).ready(function () {
    ...
    $('.hero__container').on('afterChange', function (event, slick, currentSlide) {
